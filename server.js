...
app.post('/deleteNote', function (request, result) {
    var noteId = request.body._id;
    
    // below code was used to test deleting with a string
    // var objectToDelete = request.body._id;
    ...
    Schema.findByIdAndRemove(noteId, function (error, todo){
        if (error) {
            console.log("sorry couldn't find anything or error >" + error);
        } else {
